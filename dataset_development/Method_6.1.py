...
# FUNCTION: N/A
#
# DESCRIPTION: Calculates the 'Method 6.1' score for a  multi-band DxV raster
...
if AC_width == DV_width and AC_height == DV_height:
    ...
    initial_score = np.nanmean(raster_values, axis = 0)

    # Overrides
    # ---------
    # Read DV rasters back in for conditionals (clean up this ineffciency if finalizing tool)
    # 50% grid
    DV_50_band = DV_input_dataset.GetRasterBand(1)
    DV_50_array = DV_50_band.ReadAsArray()
    # 1% grid
    DV_1_band = DV_input_dataset.GetRasterBand(5)
    DV_1_array = DV_1_band.ReadAsArray()

    condition1 = (AC_array > 48) & (DV_50_array > 0.5)
    condition2 = (AC_array >= 1) & (DV_1_array > 0.5) & (initial_score < 3)

    # Apply conditional calculations
    final_score = np.where(condition1, 10,
                    np.where(condition2, 3, 
                        initial_score))
